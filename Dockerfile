...
COPY --from=builder --chown=reactapp:nodejs /usr/src/app/packages/client/dist ./packages/client/dist/
COPY --from=builder --chown=reactapp:nodejs /usr/src/app/packages/server/dist ./packages/server/dist/
COPY --from=builder --chown=reactapp:nodejs /usr/src/app/node_modules ./node_modules/
COPY --from=builder --chown=reactapp:nodejs /usr/src/app/package.json ./

COPY --chown=reactapp:nodejs images ./images/
COPY conf/ /tmp/conf/
